...
function secure_exception_handler(Throwable $e): void {
    ...
    $isLocal = in_array($_SERVER['HTTP_HOST'] ?? '127.0.0.1', ['localhost', '127.0.0.1', '::1']);
    ...
    if ($isJson) {
        ...
        echo json_encode([
            'success' => false,
            'error'   => $isLocal ? ('Server error: ' . $e->getMessage()) : 'An internal error occurred. Please try again later.'
        ]);
        exit;
    }
